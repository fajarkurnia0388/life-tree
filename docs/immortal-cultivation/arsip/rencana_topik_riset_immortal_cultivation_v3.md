# Rencana Topik Riset Terpisah V3 — Immortal Cultivation / Xianxia / Xiuzhen

Versi ini adalah **gabungan revisi terbaik** dari evaluasi V2, terutama dari:
- **Evaluator A** → paling kuat pada arsitektur konten, overlap, gap topik, dan ketelitian desain
- **Evaluator D** → paling kuat pada project management, effort realism, versioning, MVP, dan jalur eksekusi
- **Evaluator B** → paling tajam pada governance, done criteria, milestone, template bertingkat, dan batas domain vs peran

V3 mempertahankan preferensi utama proyek ini:
> **scope tetap luas, bahkan diperluas**, tetapi sekarang diikat oleh sistem kerja, aturan status, owner concept, dan milestone yang lebih keras.
...
Kalau proyek berhenti di tengah, subset ini harus tetap berdiri sebagai produk utuh:
...
Ini adalah produk minimum yang masih akan sangat berguna untuk:
- penulis
- worldbuilder
- pembaca yang ingin memahami struktur cultivation
...
# 12. PRIORITAS EKSEKUSI REVISI V3
...
# 13. KEPUTUSAN DESAIN YANG HARUS DIKUNCI SEBELUM MENULIS FILE BERAT

1. **Korpus benchmark**: 10–20 karya utama apa yang akan dipakai terus-menerus?
2. **Bahasa terminologi**: Mandarin primer? campuran? format penulisan istilah bagaimana?
3. **Pembaca target**: untuk diri sendiri, tim kolaboratif, atau publik?
4. **Throughput realistis**: berapa jam per minggu dan berapa file per bulan?
5. **File pilot**: file mana yang dipakai untuk menguji template penuh? (disarankan `SYS-004` atau `CORE-001` versi Level 2 dulu)

---

# 14. KESIMPULAN V3

V3 sengaja tidak menjadi lebih kecil.

Ia menjadi:
- **lebih luas**, 
- **lebih sistematis**, 
- **lebih siap dikembangkan**,
- tetapi juga **lebih jujur** bahwa proyek seperti ini membutuhkan governance, milestone, dan disiplin revisi.
...
> **V2 = arsitektur arsip**
>
> **V3 = arsitektur arsip + governance proyek + jalur eksekusi nyata**

Kalau tujuanmu memang membuat riset cultivation yang sangat lengkap, maka V3 adalah versi paling dekat ke bentuk “ensiklopedia modular” yang benar-benar bisa tumbuh tanpa cepat ambruk oleh overlap dan scope.

---

# 15. LANGKAH PALING LOGIS SETELAH V3

Jika melanjutkan, urutan terbaik sekarang adalah:

1. buat `META-001_master_index_dan_status_proyek.md`
2. buat `META-002_metodologi_korpus_dan_konvensi_riset.md`
3. buat `META-003_batas_scope_owner_matrix_dan_relasi_antar_file.md`
4. buat `META-004_korpus_benchmark_dan_alasan_pemilihan.md`
5. buat `META-008_estimasi_effort_dan_throughput_planning.md`
6. baru uji template dengan satu file pilot Level 2
7. setelah itu, serang `CORE-001_riset_qi_komprehensif.md`
